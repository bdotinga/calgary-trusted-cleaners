    ...
    <div className="space-y-6 max-w-full">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="border-b border-white/8">
                <tr>
                  {['#','Name','Title','Company','Office Phone','Mobile','Email','Best Time','Last Contact','Next Follow-Up','Notes',...(isAdmin?['']:[])]
      ...
      <Modal title={editing ? 'Edit Contact' : 'Add Contact'} open={modalOpen} onClose={closeModal} wide>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="Name" required><Input value={form.name} onChange={e => setField('name', e.target.value)} placeholder="Full name"/></FormField>
          <FormField label="Title / Role"><Input value={form.title} onChange={e => setField('title', e.target.value)} placeholder="Project Manager, VP…"/></FormField>
          <FormField label="Company"><Input value={form.company} onChange={e => setField('company', e.target.value)} placeholder="Company name"/></FormField>
          ...
          <FormField label="Office Phone"><Input type="tel" value={form.office_phone} onChange={e => setField('office_phone', e.target.value)} placeholder="(403) 000-0000"/></FormField>
          <FormField label="Mobile"><Input type="tel" value={form.mobile} onChange={e => setField('mobile', e.target.value)} placeholder="(403) 000-0000"/></FormField>
          <div className="sm:col-span-2">
            <FormField label="Email"><Input type="email" value={form.email} onChange={e => setField('email', e.target.value)} placeholder="email@company.com"/></FormField>
          </div>
          <FormField label="Last Contact Date"><Input type="date" value={form.last_contact_date} onChange={e => setField('last_contact_date', e.target.value)}/></FormField>
          <FormField label="Next Follow-Up"><Input type="date" value={form.next_followup} onChange={e => setField('next_followup', e.target.value)}/></FormField>
          <div className="sm:col-span-2">
            <FormField label="Notes"><Textarea value={form.notes} onChange={e => setField('notes', e.target.value)} placeholder="Notes about this contact…"/></FormField>
          </div>
        </div>
